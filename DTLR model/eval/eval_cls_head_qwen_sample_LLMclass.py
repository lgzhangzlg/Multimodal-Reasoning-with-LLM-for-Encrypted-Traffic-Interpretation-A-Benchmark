...
#
# v2 改动：
# 1. class 字段不再由 LLM 生成，由 NetMamba top-1 预测直接填入
# 2. evaluate() 里推理前单独跑一次 NetMamba 拿到 pred_classes
# 3. 生成后把 class 拼到 JSON 最前面
# 4. calculate_metrics 里 gt_class 从 sample["class"] 字段读取（不再从 target JSON 里取）
# 5. EvalDataset.__getitem__ 兜底 user_text 同步更新为新格式（含 <cls_placeholder>）
#
# Usage:
...
#         # 直接从 sample["class"] 读取，不再解析 target JSON
...
#             "gt_class":       sample.get("class", "unknown"),  # ← 新增，用于评估
...
#     gt_classes      = [item["gt_class"]       for item in batch]  # ← 新增
...
# # ============================================================
#
# def load_model(checkpoint_path: str, device: str = "cuda") -> TinyLlavaForConditionalGeneration:
...
#
#     vt_path = os.path.join(parent_dir, "vision_tower", "pytorch_model.bin")
#     if os.path.exists(vt_path):
#         state = torch.load(vt_path, map_location="cpu", weights_only=False)
#         vt    = model.vision_tower._vision_tower
#         if "head.weight" in state:
#             n_saved   = state["head.weight"].shape[0]
#             n_current = vt.head.weight.shape[0]
#             if n_saved != n_current:
#                 import torch.nn as nn
#                 vt.head = nn.Linear(vt.head.in_features, n_saved)
#         vt.load_state_dict(state, strict=False)
#         print("  vision_tower loaded")
#
...
#         _, logits = model.vision_tower._vision_tower(
...
#     gt_classes: List[str],  # ← 新增，直接用 sample["class"]
# ) -> Dict:
...
#     )
#
#     # evaluate 返回 4 个值，多了 gt_classes_all
...
#     )
#
#     # calculate_metrics 新增 gt_classes 参数
    ...
    def __getitem__(self, i) -> Dict[str, Any]:
        ...
        if not user_text:
            user_text = (
                f"{DEFAULT_IMAGE_TOKEN}\n"
                "Above are the raw traffic byte features extracted from a network flow.\n\n"
                "Based on traffic pattern analysis, this flow is tentatively classified as: "
                "<cls_placeholder> (confidence: <conf_placeholder>). "
                "Use the byte features above to confirm or correct this prediction.\n\n"
                "Based on the traffic byte features and the above classification result, "
                "return a single JSON object with the following keys:\n"
                f"- class: the traffic category name. Must be exactly one of: Browsing, Chat, Email, FTP, P2P, Streaming, VoIP.\n\n"
                "- traits: an object describing byte-level characteristics\n"
                "- evidence: a list of 2~4 concrete byte-level observations\n"
                "- description: a paragraph of 2~3 sentences summarizing the traffic\n"
                "- notes: a single security-relevant observation or recommendation"
...
def load_model(checkpoint_path: str, device: str = "cuda") -> TinyLlavaForConditionalGeneration:
    ...
    model.vision_tower._load_model(
        ...
        pretrained_vision_tower_path="/root/autodl-tmp/Tiny_LLaVA/third_model/NetMamba/finetune_train_out/ISCXVPN2016/ISCXVPN2016_npy_split_npy_v3_balacned_200_6000_train_test_splited_epochs60/checkpoint-best.pth"
    )
    ...
    ckpt_tokenizer = AutoTokenizer.from_pretrained(checkpoint_path)
    model.tokenizer = ckpt_tokenizer
    print(f"  tokenizer replaced from {checkpoint_path}, vocab_size={len(ckpt_tokenizer)}")
    ...
    conf_pid = ckpt_tokenizer.convert_tokens_to_ids("<conf_placeholder>")
    print(f"  <conf_placeholder> id={conf_pid}")
    assert isinstance(conf_pid, int), f"<conf_placeholder> not in tokenizer! conf_pid={conf_pid}"
...
def evaluate(
    ...
    for batch in tqdm(dataloader, desc="Evaluating"):
        ...
        # ── 1. 单独跑 NetMamba 拿到 top-1 预测类别，用于最终 JSON 拼接 ──
        # ── NetMamba 预测在 prepare_inputs_labels_for_multimodal 内部自动完成 ──
        # <cls_placeholder> 会被替换为 NetMamba top-1 预测类别的 special token embedding
...
def calculate_metrics(
    ...
    # class 由 LLM 预测输出，required_fields 包含 class
