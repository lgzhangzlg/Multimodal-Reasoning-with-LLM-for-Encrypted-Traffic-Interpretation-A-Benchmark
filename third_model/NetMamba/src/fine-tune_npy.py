...
# import models_net_mamba_28packet as models_net_mamba
...
def build_dataset_npy(data_split, args):
    mean = [0.5]
    std = [0.5]
...
def main(args):
    ...
    '''数据集'''
    dataset_train = NPYPipelineDataset(os.path.join(args.data_path, 'train'), transform=transform_train)
    dataset_val = NPYPipelineDataset(os.path.join(args.data_path, 'test'), transform=transform_train)
    dataset_test = NPYPipelineDataset(os.path.join(args.data_path, 'test'), transform=transform_train)
    ...
    if args.smoothing > 0.:
        ...
        # criterion = FocalLoss(gamma=4, alpha=0.25)
        # criterion = [LabelSmoothingCrossEntropy(smoothing=args.smoothing),
        #                  ECCLoss(119, dim=2048).to(device)]
    else:
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, device, data_loader_train)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # cm = test_stats.get("cm", None)
        # if cm is not None:
        #     # 创建一个图形
        #     plt.figure(figsize=(10, 8))
        #
        #     # 绘制热力图
        #     sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=range(cm.shape[1]),
        ...
        #     plt.title(f'{epoch}_Confusion Matrix', fontsize=18)
        #
        #     # 保存混淆矩阵图像
        #     plt.savefig(f"/root/autodl-tmp/Net-Mamba/show_fig/epoch_{epoch}_Confusion Matrix")
        #     plt.close()

        # 保存 per-class precision 和 recall 到文本文件
        # pre_per_class = test_stats.get('pre_per_class')
        # rec_per_class = test_stats.get('rec_per_class')

        # if pre_per_class is not None and rec_per_class is not None:
        #     with open(f'/root/autodl-tmp/Net-Mamba/out_fine_tune/show_fig_cstnet-tls_1.3/precision_recall_epoch_{epoch}.txt',
        #               'a') as f:
        #         f.write(f"Epoch {epoch}\n")
        #         f.write("Class\tPrecision\tRecall\n")
        #         for i in range(len(pre_per_class)):
        #             f.write(f"{i}\t{pre_per_class[i]:.4f}\t{rec_per_class[i]:.4f}\n")
        #         f.write("\n")
        ...
        log_stats = {'epoch': epoch,
                     **{f'train_{k}': v if not isinstance(v, np.ndarray) else v.tolist() for k, v in
                        train_stats.items()},
                     **{f'valid_{k}': v if not isinstance(v, np.ndarray) else v.tolist() for k, v in
                        test_stats.items()},
