...
class LazySupervisedDataset(Dataset):
    ...
    def _build_conversations(self, sample):
        """
        Human: fixed instruction requiring PURE JSON only.
        ...
        if not assistant_text:
            raise KeyError(
                "jsonl sample missing required field: target. "
                "Please use the generated train_byte_grounded.jsonl and ensure each row has 'target'."
            )
