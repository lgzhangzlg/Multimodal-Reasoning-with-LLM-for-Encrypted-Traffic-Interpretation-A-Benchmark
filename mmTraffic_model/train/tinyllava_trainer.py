...
def get_modality_length_grouped_indices(lengths, batch_size, world_size, generator=None):
    ...
    # [MOD] 仍然保留这个约束：0 表示非法长度
    assert all(l != 0 for l in lengths), "Should not have zero length."

    megabatch_size = world_size * batch_size

    # [MOD] 不用 zip(*)，避免空列表直接 ValueError
    mm_pairs = [(i, l) for i, l in enumerate(lengths) if l > 0]
    lang_pairs = [(i, -l) for i, l in enumerate(lengths) if l < 0]

    # [MOD] 允许某一种不存在：退化为“单模态长度分桶”
    if len(mm_pairs) == 0 and len(lang_pairs) == 0:
        # 理论上不会发生（因为 length 不能为 0），但防御一下
        return list(range(len(lengths)))

    if len(lang_pairs) == 0:
        mm_indices, mm_lengths = zip(*mm_pairs)
        order = get_length_grouped_indices(list(mm_lengths), batch_size, world_size, generator=generator)  # [MOD] 传入generator
        return [mm_indices[i] for i in order]

    if len(mm_pairs) == 0:
        lang_indices, lang_lengths = zip(*lang_pairs)
        order = get_length_grouped_indices(list(lang_lengths), batch_size, world_size, generator=generator)  # [MOD] 传入generator
        return [lang_indices[i] for i in order]

    # [MOD] 混合情况：保持原有“mm/lang 各自分桶 + megabatch 混洗”策略
    mm_indices, mm_lengths = zip(*mm_pairs)
    lang_indices, lang_lengths = zip(*lang_pairs)

    mm_order = get_length_grouped_indices(list(mm_lengths), batch_size, world_size, generator=generator)    # [MOD]
    lang_order = get_length_grouped_indices(list(lang_lengths), batch_size, world_size, generator=generator)  # [MOD]
    ...
    # [MOD] 处理只有1个megabatch甚至为空的边界（避免 [-1] 崩）
    last_mm = mm_megabatches[-1] if len(mm_megabatches) > 0 else []
    last_lang = lang_megabatches[-1] if len(lang_megabatches) > 0 else []

    additional_batch = list(last_mm) + list(last_lang)
    megabatches = mm_megabatches[:-1] + lang_megabatches[:-1]

    if len(megabatches) > 0:
        megabatch_indices = torch.randperm(len(megabatches), generator=generator)
        megabatches = [megabatches[i] for i in megabatch_indices]

    # [MOD] 先凑满一个完整 megabatch 放到最前面，减少“最后一个小batch”影响吞吐
    if len(additional_batch) >= megabatch_size:
        megabatches = [additional_batch[:megabatch_size]] + megabatches
        additional_batch = additional_batch[megabatch_size:]
