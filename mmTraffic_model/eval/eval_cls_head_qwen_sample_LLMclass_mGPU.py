    ...
    for batch in tqdm(dataloader, desc=f"[{device}] Evaluating"):
        ...
        if has_cls_head:
            ...
            if cls_logits is not None:
                ...
                # 没有 cache，填 -1
                cls_head_preds.extend([-1] * images.shape[0])
...
def calculate_metrics(
    ...
    def _normalize(s):
        ...
        e.g. ' "Tencent  Video." ' → 'tencent video'
        e.g. 'UC_Browser'         → 'uc browser'
        """
        if not isinstance(s, str):
            return ""
        import re as _re
        s = s.strip().strip('"\'').strip()      # 去首尾空格和引号
        s = s.lower()                            # 统一小写
        s = _re.sub(r'[._\-]+', ' ', s)         # 下划线/点/连字符 → 空格
        s = _re.sub(r'[^\w\s]', '', s)           # 去掉标点符号
        s = _re.sub(r'\s+', ' ', s).strip()      # 合并多余空格
        return s
    ...
    for pred, gt_str, gt_class in zip(predictions, ground_truths, gt_classes):
        ...
        if pred_json is not None and isinstance(pred_json, dict):
            ...
            pred_class = pred_json.get("class", "")
            norm_pred = _normalize(pred_class)
            ...
            if norm_pred == norm_gt:
                fuzzy_match_count += 1
            # 包含匹配（pred 包含 gt 或 gt 包含 pred，捕获 "Tencent Video App" vs "Tencent Video"）
    ...
    if cls_head_accuracy is not None:
        ...
        print(f"\n[CLS_HEAD] Test accuracy: {cls_head_accuracy:.2f}% ({cls_head_correct}/{cls_head_total})")
...
def main():
    ...
    # 优先从 cls_head 目录加载 class_list.json（与训练时保持一致）
    # 若无，则从测试数据中按 sorted() 构建（与训练时 dataset.py 逻辑一致）
    class_list = None
    ...
    if class_list is None:
        class_list = sorted(set(s.get("class", "unknown") for s in full_data))
        print(f"[INFO] class_list built from data: {len(class_list)} classes (sorted, same as training)")
