...
def load_model(checkpoint_path: str, device: str = "cuda", vision_tower_path: str = None):
    ...
    if os.path.exists(llm_path):
        model.language_model = model.language_model.from_pretrained(
            llm_path, local_files_only=True
        )
    else:
...
def evaluate(
    ...
    for batch in tqdm(dataloader, desc=f"[{device}] Evaluating"):
        ...
        # ── Text-Only Anchor: 保留class anchor，清零NetMamba字节特征 ──
        # 用零向量替换真实图像输入，使connector投影结果为零向量
        # pred_classes已由真实images计算得到，class anchor不受影响
        images_zeroed = torch.zeros_like(images)
