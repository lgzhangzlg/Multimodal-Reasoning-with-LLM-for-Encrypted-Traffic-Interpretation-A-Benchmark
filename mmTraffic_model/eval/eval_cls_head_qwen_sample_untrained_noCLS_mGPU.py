...
Untrained variant, no CLS injection (ablation baseline):
  ...
  - Class injection: NONE — <cls_placeholder> stripped from prompt entirely
  - Focus:     generation quality (json_parse_rate, field_completeness_rate)
...
class EvalDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, Any]:
        ...
        # [noCLS variant] Strip <cls_placeholder> line from user_text if present
        if user_text:
            user_text = re.sub(
                r"The traffic has been classified as:.*?<cls_placeholder>\.?\s*\n?",
                "",
                user_text,
            ).strip()
        if not user_text:
            user_text = (
                ...
                "Based on the traffic byte features, "
