...
# ============================================================
# user_text 模板（v2：使用 <cls_placeholder> 占位符）
# ============================================================

def build_user_text(class_list: List[str]) -> str:
    """
    训练和推理时的 user_text 模板完全一致，都使用 <cls_placeholder>。
    在 prepare_inputs_labels_for_multimodal 里：
      - 训练时：将 <cls_placeholder> 的 embedding 替换为 ground truth 类别的 special token embedding
      - 推理时：将 <cls_placeholder> 的 embedding 替换为 NetMamba top-1 预测类别的 special token embedding
    """
...
def convert_pcaps_to_npy(
    ...
    if workers > 1:
        ...
        with multiprocessing.Pool(processes=workers) as pool:
            results_iter = pool.imap_unordered(_npy_worker, tasks, chunksize=chunksize)
            for res in tqdm(results_iter, total=len(tasks), desc="pcap→npy", ncols=80):
                if res["status"] in ("ok", "exists") and res["npy"]:
                    pcap_to_npy[res["pcap"]] = res["npy"]
                    if res["status"] == "ok":
                        ok += 1
                    else:
...
def build_jsonl_rows(
    ...
    # 使用 tqdm 显示进度条
    for pcap_path in tqdm(pcap_files, desc="Processing PCAP files", unit="file"):
