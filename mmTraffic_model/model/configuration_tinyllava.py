...
class DummyVisionConfig(PretrainedConfig):
    model_type = "netmamba_vision"

    def __init__(self, hidden_size=512, model_name_or_path="netmamba", model_name_or_path2="", **kwargs):
...
class TinyLlavaConfig(PretrainedConfig):
    ...
    def load_from_config(self, config):
        ...
        self.tokenizer_model_max_length = getattr(config, 'model_max_length', 3072)
    ...
    def to_dict(self):
        ...
        # 如果 vision_config 是 DummyVisionConfig 类的实例，就将其转换为字典
        if isinstance(self.vision_config, DummyVisionConfig):
            d['vision_config'] = self.vision_config.to_dict()

        return d

    def _load_vision_config(self, vision_config=None):
        vt = (self.vision_model_name_or_path or '').split(':')[-1]
        ...
        if vt in [None, '']:
            ...
            # 👇 给一个“假的”vision_config，只要有 hidden_size 就行


            self.vision_config = DummyVisionConfig()
            self.vision_config.model_name_or_path = 'netmamba'
            self.vision_config.model_name_or_path2 = ''
            # 这里要和 NetMamba 输出维度对齐
            self.vision_hidden_size = 512  # 如果你最后输出 dim=512
            return
        else:
